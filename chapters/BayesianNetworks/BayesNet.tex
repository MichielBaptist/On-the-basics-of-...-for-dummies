...
\begin{equation}\label{eq:message_var_to_fact_}\begin{split}
\mu_{X_{ij} \rightarrow f_i}(X_{ij}) = \sum_{V_{ij} \setminus \{X_{ij}\}}
			F_{ij}(X_{ij}, V_{ij})
\end{split}\end{equation}
...
\begin{tikzpicture}
	...
	\node[fn, label=right:{$f_i$}](X) at (0,0){};
	\node[vn] (fi) at (0, -1){$X_{ij}$};
	
	\node[fn, label=right:{$f_{ij1}$}](Xi1) at (-1.6, -3) {};
	\node[](dots1) at (0, -3) {$\dots$};
	\node[fn, label=right:{$f_{ijk}$}](Xim) at (1.6, -3) {};
	
	\node[vn](fi11) at (-3, -5){$X_{ij1}^1$};
	\node[] (dots2) at (-2, -5){$\dots$};
	\node[vn](fi1k) at (-1, -5){$X_{ij1}^s$};
	
	\node[vn](fim1) at (1, -5){$X_{ijk}^1$};
	\node[] (dots3) at (2, -5){$\dots$};
	\node[vn](fimk2) at (3, -5){$X_{ijk}^{s'}$};
	...
	\draw[rounded corners, ultra thick, gray] 
		...
		(-0.8, -0.5) --
		(0, -0.5);
	\node[text = gray](Gi) at (-0.8, -1){$G_{ij}$};
	...
	\draw[rounded corners, ultra thick, blue, opacity=0.3] 
		...
		(-2.1, -2.4) --
		(-1.8, -2.4);
	\node[text = blue, opacity = 0.3](Gi1) at (-2.3, -3.5){$G_{ij1}$};
	...
	\draw[rounded corners, ultra thick, blue, opacity=0.3] 
		...
		(2.1, -2.4) --
		(1.8, -2.4);
	\node[text = blue, opacity = 0.3](Gi1) at (2.4, -3.5){$G_{ijk}$};
	
\end{tikzpicture}

\caption{Recursively partitioning of $G_{ij}$.}
\label{fig:subtree_Xij}
\end{figure}

\noindent
The resulting partition is then again decided according to the sub
graphs $S_{G_{ij}}(X_{ij}) = \{G_{ij1}, \dots, G_{ijk}\}$. Where each
sub graph $G_{ijk}$ is defined as:
...
\begin{enumerate}
\item The marginal probability $p(X)$ is simply the product of
all incoming messages from factor nodes:
\begin{multicols}{2}
\begin{equation}
p(X) = \prod_{f_i \in N(X)} \mu_{f_i \rightarrow X}(X)
\end{equation}
\break
\begin{tikzpicture}
	[
	...
	\node[fn, label=above:{$f_1$}] (f1) at (0, 0.5){};
	\node[fn, label=above:{$f_2$}] (f2) at (0, -0.5){};
	\node[fn, label=above:{$f_3$}] (f3) at (4, 0){};
	
	\node[vn] (X) at (2, 0){$X$};
	
	\draw[ultra thick, green, ->] (f1) -- node[above]{$\mu_1$}(X);
	\draw[ultra thick, green, ->] (f2) -- node[above]{$\mu_2$}(X);
	\draw[ultra thick, green, ->] (f3) -- node[above]{$\mu_3$}(X);
	
	\path (X) edge [blue, ultra thick,loop above, opacity = 0.7] 
			node{$\prod \mu_i$} (X);
	
\end{tikzpicture}
\end{multicols}

\item The messages from factor node $f_i$ to variable $X$ is simply
the product of all incoming message and the factor $f_i(\textbf{X}_i)$:
\begin{multicols}{2}
\begin{equation}\begin{split}
\mu_{f_i \rightarrow X}(X) = f_i(\textbf{X}_i) * \\
	\prod_{\substack{X_j \in N(f_i) \\ X_j \neq X}} 
	\mu_{X_j \rightarrow f_i}(X_j)
\end{split}\end{equation}
\break
\hspace{4em}
\begin{tikzpicture}
	...
	\node[vn] (1) at (0, 0.5){$X_1$};
	\node[vn] (2) at (0, -0.5){$X_2$};
	\node[vn] (X) at (4, 0){$X$};
	
	\node[fn, label=below:{$f_i$}] (f) at (2, 0){};
	
	\draw[ultra thick, green, ->] (1) -- node[above]{$\mu_1$}(f);
	\draw[ultra thick, green, ->] (2) -- node[above]{$\mu_2$}(f);
	\draw[ultra thick, red, ->, opacity =0.7] (f) -- node[above]{$\mu(X)$}(X);
	
	\path (f) edge [blue, ultra thick,loop above, opacity = 0.7] 
			node{$f\prod \mu_i$} (f);
	
\end{tikzpicture}
\end{multicols}

\item The messages from variable node $X_i$ to factor node $f$ is simply
the product of all incoming messages:
\begin{multicols}{2}
\begin{equation}
\mu_{X_i \rightarrow f}(X_i) = 
	\prod_{\substack{f_j \in N(X_i) \\ f_j \neq f}} 
	\mu_{f_j \rightarrow X_i}(X_j)
\end{equation}
\break
\begin{tikzpicture}
	[
	...
	\node[fn, label=above:{$f_1$}] (f1) at (0, 0.5){};
	\node[fn, label=above:{$f_2$}] (f2) at (0, -0.5){};
	\node[fn, label=above:{$f$}] (f) at (4, 0){};
	
	\node[vn] (X) at (2, 0){$X$};
	
	\draw[ultra thick, green, ->] (f1) -- node[above]{$\mu_1$}(X);
	\draw[ultra thick, green, ->] (f2) -- node[above]{$\mu_2$}(X);
	\draw[ultra thick, red, ->, opacity = 0.7] (X) -- node[above]{$\mu(X)$}(f);
...
\noindent
As our derivation of these messages was based on recursively 
partitionion all the factors in the sub graph, this recursion 
clearly must come to an end. Indeed consider a message 
sent from a factor node $f_i$ to a variable 
node $X$, however $f_i$ has no other connections except fom $X$.
This situation is depicted in figure \ref{fig:message_base_case_1}.
...
\begin{tikzpicture}
	...
	\node[fn, label=above:{$f_i$}] (f) at (0,0){};
	\node[vn] (X) at (3,0){$X$};
	
	\draw[-] (X) -- (f);
	\draw[->, ultra thick, blue, opacity = 0.8] (0.3, 0.3) 
	-- node[above]{$\mu_{f_i \rightarrow X} = f_i(X)$} (2.5, 0.3);
	
	\draw[dotted, ultra thick, red , opacity=0.6] (3.5, -1) ..
		controls (2,-0.5) and (2, 0.5) ..
		(3.5, 1);
		
	\draw[-] (X) -- (3.5, 0.5);
	\node[] (dd) at (3.5, 0){$\vdots$};
	\draw[-] (X) -- (3.5, -0.5);
...
\begin{tikzpicture}
	...
	\node[vn] (X) at (0,0){$X_i$};
	\node[fn, label=above:{$f$}] (f) at (3,0){};
	
	\draw[-] (X) -- (f);
	\draw[->, ultra thick, blue, opacity = 0.8] (0.5, 0.3) 
	-- node[above]{$\mu_{X_i \rightarrow f} = 1$} (2.7, 0.3);
