...
\begin{tikzpicture}
	...
	\node[fn, label=above:{$f$}] (f) at (0,0){};
	\node[vn] (X) at (3,0){$X$};
	
	\draw[-] (X) -- (f);
	\draw[->, ultra thick, blue, opacity = 0.8] (0.3, 0.3) 
	-- node[above]{$\mu_{f \rightarrow X} = f(X)$} (2.5, 0.3);
\end{tikzpicture}
\caption{First base case. A message is sent from factor node $f$
to $X$, where $f$ only has $X$ as neighbour. I.e. $N(f) = \{X\}$.}
